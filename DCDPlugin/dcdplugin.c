...
void ComputeCOM(double** COM, double* Mass, const dcdhandle* dcd, const double TotalMass, const int natoms, const int tframe) {
  int i;

  for(i = 0; i < natoms; i++) {
    COM[tframe][0] += dcd->x[i] * Mass[i];
    COM[tframe][1] += dcd->y[i] * Mass[i];
    COM[tframe][2] += dcd->z[i] * Mass[i];
  }

  for(i = 0; i < 3; i++)
    COM[tframe][i] /= TotalMass;

}

bool WriteDatatoFile(double** data, const int size1, const int size2, const char* fname) {
  int i = 0, j = 0;
...
double* ReadMass(char* psffile) {
  // written by Yuri Sereda
  char psf[256];
  FILE* fd;
  char line[512];
  double* masses;
  ...
  if( fd = fopen(psf,"r") ) {
      ...
      fgets(line, 512, fd);
      fgets(line, 512, fd);
      fscanf(fd,"%d",&i); // remarks
      fgets(line, 512, fd);
    
      // skip remarks
      for(j=0; j<i; j++) fgets(line, 512, fd);

      // read number of atoms
      fscanf(fd,"%d",&Natoms);
      fgets(line, 512, fd);
    
      masses = (double*) malloc(sizeof(double)*Natoms);

      for(i = 0; i < Natoms; i++) {
        fgets(line, 512, fd);
        if( sscanf(line,"%*51c%lf",&masses[i]) )
          ;
        else 
          break;
      }
...
double ComputeTotalMass(double* Mass, const int Natoms) {
  int i;
  double TotalMass = .0;
...
int main(int argc, char *argv[]) {
  ...
  if(argc >= 4) {
    printf("*********************************************\n");
    printf("DCDplugin hacked by -=Andrew Abi-Mansour=- v1.0\n");
    printf("*********************************************\n");
    natoms = 0;
    v = open_dcd_read(argv[1], "dcd", &natoms);
    if (!v) {
    ...
    char *psffile = argv[2], *ofname = argv[3];

    starttime = time_of_day();
    timestep.coords = (float *)malloc(3*sizeof(float)*natoms);
    int tframe = 0;

    double** COM = (double**) malloc(sizeof(double*)*dcd->nsets);
    double* Mass = ReadMass(psffile);
    double TotalMass = ComputeTotalMass(Mass, natoms);

    for(i = 0; i < dcd->nsets; i++)
      COM[i] = (double*) malloc(sizeof(double)*3);
    ...
    for (i = 0; i < dcd->nsets; i++) {
      ...
      dcdhandle* dcd_tmp = (dcdhandle*) v;
      /* do computations here */
      ComputeCOM(COM, Mass, dcd_tmp, TotalMass, natoms, tframe++);
    ...
    WriteDatatoFile(COM,dcd->nsets,3,ofname);
